...
(function () {
  ...
  function initDonationForm() {
    ...
    // Form submit — calls Convex createPayment action via HTTP API
    form.addEventListener('submit', function (e) {
      ...
      var donorName = document.getElementById('firstName').value || 'Anonymous';
      var donorEmail = document.getElementById('email').value || 'anonymous@example.com';
      ...
      .then(function (data) {
        if (data.value && data.value.checkoutUrl) {
          // Store donation info locally before redirecting
          ...
          // Redirect to Triple-A hosted checkout
      ...
      .catch(function (err) {
        ...
        alert('Payment could not be processed: ' + err.message + '\nPlease try again.');

        // Fallback: still show the thank you modal for demo purposes
        localStorage.setItem('hasDonated', 'true');
        localStorage.setItem('donationAmount', selectedAmount);
        localStorage.setItem('donorName', donorName);
