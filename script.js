...
(function () {
  ...
  function initDonationForm() {
    ...
    // Form submit — create Coinremitter invoice via Convex
    form.addEventListener('submit', function (e) {
      ...
      var dedicateName = '';
      var dedicateCheckboxEl = document.getElementById('dedicateGift');
      if (dedicateCheckboxEl && dedicateCheckboxEl.checked) {
        dedicateName = (document.getElementById('dedicateName') || {}).value || '';
      }

      var client = getConvexClient();
      if (!client) {
        console.error('[Donation] Convex client not available — falling back to local mode');
        completeDonationLocally(selectedAmount, firstName);
        return;
      }
      ...
      client.action('payments:createCoinremitterInvoice', actionArgs)
        .then(function (result) {
          console.log('[Donation] Invoice created:', JSON.stringify(result));

          // Store donation info locally
          localStorage.setItem('hasDonated', 'true');
          localStorage.setItem('donationAmount', selectedAmount);
          localStorage.setItem('donorName', firstName);

          // Redirect to Coinremitter invoice payment page
          if (result && result.invoiceUrl) {
            window.open(result.invoiceUrl, '_blank');
          }

          // Unlock comments and show confirmation
          initCommentSection();
        ...
        .catch(function (err) {
          console.error('[Donation] Failed to create invoice:', err);
          alert('Unable to create payment invoice. Please try again. Error: ' + (err.message || err));
          submitBtn.disabled = false;
          updateSubmitButton();
        });
    });

    // Fallback for when Convex is unavailable
    function completeDonationLocally(amount, name) {
      localStorage.setItem('hasDonated', 'true');
